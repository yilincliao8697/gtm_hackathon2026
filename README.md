...
`POST /run/{run_id}/generate-outreach?limit=N` calls `outreach.generate_for_contacts` to produce per-contact sales emails. Drafts are persisted to the `outreach` table and exported in the CSV. Requires `OPENAI_API_KEY`; `TAVILY_API_KEY` is optional and adds a brief web-research summary to each prompt. Configure these in `gtm/.env` or `research/.env`.
